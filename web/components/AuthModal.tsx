...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  // Enterprise Mobile Virtual Keyboard Adjustment State
  ...
  // Industry Standard: Visual Viewport API for software keyboard detection
  useEffect(() => {
    ...
    const handleVisualViewportChange = () => {
      if (window.visualViewport) {
        ...
        if (keyboardHeight > 100) {
          setKeyboardOffset(Math.min(keyboardHeight, window.innerHeight * 0.45));
        } else {
          setKeyboardOffset(0);
        }
      }
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[9999] flex flex-col justify-end sm:justify-center items-center p-0 sm:p-4 bg-black/80 backdrop-blur-md overflow-y-auto transition-all duration-300 animate-fade-in"
      style={{
    ...
    >
      <div
        ref={modalCardRef}
        className="relative w-full sm:max-w-md bg-zinc-950 border border-zinc-800 rounded-t-3xl sm:rounded-2xl shadow-[0_0_50px_rgba(16,185,129,0.1)] overflow-hidden max-h-[90dvh] flex flex-col transition-transform duration-200"
      >
        {/* Mobile Drag Indicator Bar */}
        <div className="w-12 h-1 bg-zinc-800 rounded-full mx-auto mt-2.5 mb-1 sm:hidden shrink-0" />

        {/* Glowing top border accent */}
        <div className="h-1 w-full bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 shrink-0"></div>

        {/* Scrollable Modal Content */}
        <div className="overflow-y-auto flex-1">
          {/* Modal Header */}
          <div className="p-5 sm:p-6 pb-4 flex items-center justify-between border-b border-zinc-900">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-[10px] sm:text-xs text-zinc-400 font-mono">CORE ACCESS</p>
              </div>
            </div>
            <button
              onClick={onClose}
